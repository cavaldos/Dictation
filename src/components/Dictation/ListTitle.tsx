...
import { useDictation } from "~/hooks/useReduxHooks";

const ListTitle: React.FC = () => { //rightside
    ...
    return (
        <aside className="w-[99,9%] h-[99%] bg-[rgb(35,35,35)] px-2 flex flex-col">
            {/* Header */}
            ...
            {/* Blur Controls */}
            {totalCount > 0 && (
                <div className="flex-shrink-0 px-4 py-2 flex items-center gap-4">
                    {/* Primary language blur toggle */}
                    <button
                        onClick={() => setBlurPrimary(!blurPrimary)}
                        className={`flex items-center gap-1.5 text-xs px-2 py-1 rounded transition-colors ${blurPrimary
                    ...
                    {hasBilingual && (
                        <button
                            ...
                            className={`flex items-center gap-1.5 text-xs px-2 py-1 rounded transition-colors ${blurSecondary
            ...
            <div ref={listRef} className="flex-1 overflow-y-auto bg-[rgb(25,25,25)] border rounded-md ">
                {subtitles.length === 0 ? (
                        ...
                        {subtitles.map((subtitle, index) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full px-4 py-3 text-left transition-colors hover:bg-notion-hover rounded-lg ${isCurrent ? 'bg-primary/20 border-l-2 border-primary' : ''
                                ...
                                >
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            {/* Primary Text */}
                                            <div
                                                className={`text-sm transition-all duration-200 ${isCompleted
                                            ...
                                            {subtitle.translatedText && (
                                                <div
                                                    className={`text-xs mt-1 text-yellow-500/80 transition-all duration-200 ${blurSecondary ? 'blur-sm hover:blur-none' : ''
                                                        }`}
                                                >
                                                    {subtitle.translatedText}
                                                </div>
            ...
            {/* Footer */}
            <div className="flex-shrink-0 p-3 bg-[rgb(32,32,32)] border rounded-lg">
